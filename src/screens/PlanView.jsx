...
const WEEK_NAMES = ['One', 'Two', 'Three', 'Four'];
const WEEK_DESCRIPTIONS = [
  'Focus on neural recruitment and technique.',
  'Increasing load and decreasing rest intervals.',
  'Maximum sets per muscle group.',
  'Central nervous system recovery and joint health.',
];
    ...
    <div style={{ minHeight: '100dvh', background: C.bg, color: C.text, paddingBottom: '7rem' }}>
      ...
      <div style={{ ...W, paddingTop: '3rem', paddingBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingBottom: '4px', flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: C.faint, background: `rgba(255,255,255,0.05)`, padding: '4px 10px', borderRadius: '100px' }}>
              {totalWeeks * 7} Days Duration
            </span>
          </div>
        </div>
      </div>
      ...
      <div style={{ ...W, display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {selectedPlan.weeks.map((week, wi) => {
          ...
          const weekName = WEEK_NAMES[wi] || `${week.week_number}`;
          const weekDesc = WEEK_DESCRIPTIONS[wi] || '';
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                  ...
                  <div style={{ textAlign: 'left' }}>
                    <h2 className="font-headline" style={{ fontSize: '1.1rem', fontWeight: 700, letterSpacing: '-0.01em' }}>Week {weekName}: {theme}</h2>
                    <p style={{ fontSize: '0.78rem', color: C.faint, marginTop: '3px' }}>{weekDesc}</p>
                  </div>
                </div>
                    ...
                    return (
                      <div key={session.id} style={{ borderTop: `1px solid rgba(255,255,255,0.04)` }}>
                        ...
                        >
                          {/* Day + Title */}
                          <div>
                            <p style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: C.primary, marginBottom: '4px' }}>
                              {session.day}
                            </p>
                          ...
                          {/* Exercise pills or rest description */}
                          <div>
                            {exercises.length > 0 && !isSessionExpanded ? (
                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                {exercises.map((ex, i) => (
                                  <span key={i} style={{ fontSize: '0.72rem', fontWeight: 600, padding: '4px 10px', borderRadius: '6px', background: C.highest, color: C.muted }}>
                                    {ex.name}
                                  </span>
                                ))}
                              </div>
                            ) : exercises.length === 0 ? (
                              <p style={{ fontSize: '0.85rem', color: C.faint, lineHeight: 1.55 }}>
                                {session.blocks?.[0]?.detail || ''}
                              </p>
                            ) : null}
                          </div>
                          ...
                          {exercises.length > 0 && (
                            <button
                              ...
                              style={{
                                ...
                                background: isSessionExpanded ? `rgba(255,255,255,0.06)` : 'transparent',
                                color: isSessionExpanded ? C.text : C.faint,
                              ...
                              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.14)'; e.currentTarget.style.color = isSessionExpanded ? C.text : C.faint; }}
                            >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginTop: '8px' }}>
          {[
            { label: 'Completed Days', value: completedSessions, suffix: `/ ${totalSessions}`, color: C.green },
            { label: 'Projected Burn', value: `${(totalSessions * 320 / 1000).toFixed(1)}k`, suffix: 'kcal', color: C.primary },
            { label: 'Focus Metric', value: 'Hypertrophy', color: C.text },
          ].map(({ label, value, suffix, color }) => (
            <div key={label} style={{ background: C.lowest, borderRadius: '12px', padding: '20px', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px' }}>
                ...
                {suffix && <span style={{ fontSize: '0.78rem', color: C.faint }}>{suffix}</span>}
              </div>
            </div>
          ))}
        </div>
