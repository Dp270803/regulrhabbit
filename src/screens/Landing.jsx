...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    if (isOnboardingComplete()) setShowWelcomeBack(true);
    ...
    <div style={{ background: 'var(--color-bg)', color: 'var(--color-text-1)', minHeight: '100dvh' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          {showWelcomeBack && (
            <button
              ...
              style={{ fontSize: '0.875rem', color: 'var(--color-text-2)', background: 'none', border: 'none', cursor: 'pointer' }}
            >
              Dashboard →
            </button>
          )}
      ...
      >
        ...
        <p
          ...
          style={{ fontSize: '0.72rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--color-text-3)', marginBottom: '2rem' }}
        >
        ...
        >
          No login. No guilt. No excuses. Build physical and creative habits
          using the same science elite athletes use — right on your phone.
        </p>

        <div
          className="animate-fade-in-up delay-200"
          style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center', opacity: 0, animationFillMode: 'forwards' }}
        >
          <button
            onClick={go}
            style={{
        ...
        >
          ...
          <div style={{ width: '1px', background: 'var(--color-border)', alignSelf: 'stretch' }} />
          <Stat value="7" label="Activities supported" />
        </div>
      </section>

      {/* ── Section 1: The Science — two column ── */}
      <section style={{ borderTop: '1px solid var(--color-border)', padding: '7rem 5vw' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6rem', alignItems: 'center' }}>
          ...
          <div>
            <p
              className="font-mono"
              style={{ fontSize: '0.72rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--color-text-3)', marginBottom: '1.5rem' }}
            >
              The Science
            </p>
            <h2
              className="font-display"
              style={{ fontSize: 'clamp(2rem, 3.5vw, 3rem)', lineHeight: 1.1, marginBottom: '1.5rem', color: 'var(--color-text-1)' }}
            >
          ...
          {/* Right: visual — 2-day rule illustration */}
      ...
      {/* ── Section 2: Identity — reversed two-column ── */}
      <section style={{ borderTop: '1px solid var(--color-border)', padding: '7rem 5vw' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6rem', alignItems: 'center' }}>
          {/* Left: level progression visual */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            {[
              ...
              >
                <span
                  className="font-mono"
                  style={{ fontSize: '1.4rem', fontWeight: 700, color: active ? 'var(--color-gold)' : 'var(--color-text-3)', width: '2rem', lineHeight: 1 }}
                >
                  {level}
                </span>
                <div style={{ flex: 1 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '1rem', lineHeight: 1.8, color: 'var(--color-text-2)', marginBottom: '1rem' }}>
              ...
              Every musician started with a terrible first note.
            </p>
            <p style={{ fontSize: '1rem', lineHeight: 1.8, color: 'var(--color-text-2)' }}>
              Regulr doesn't track what you do — it tracks who you're <strong style={{ color: 'var(--color-text-1)' }}>becoming</strong>.
      ...
      {/* ── Section 3: How it works — 3 cards full width ── */}
      <section style={{ borderTop: '1px solid var(--color-border)', padding: '7rem 5vw' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '4rem' }}>
            <h2
              className="font-display"
              style={{ fontSize: 'clamp(2rem, 3.5vw, 2.8rem)', lineHeight: 1.1, color: 'var(--color-text-1)' }}
            >
            ...
            {[
              {
                ...
                body: 'A quick conversation about your activity, experience level, schedule, and mindset. No sign-up. No email.',
                detail: '~2 minutes',
              },
              {
                num: '02',
                title: 'Get your 4-week plan',
                body: 'A progressive plan built around science — not generic advice. Scales with your level every week.',
      ...
      <section style={{ borderTop: '1px solid var(--color-border)', padding: '8rem 5vw', textAlign: 'center' }}>
        <div style={{ maxWidth: '640px', margin: '0 auto' }}>
          <h2
            className="font-display"
            style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)', lineHeight: 1.02, marginBottom: '1.5rem', color: 'var(--color-text-1)', letterSpacing: '-0.02em' }}
          >
            No email.<br />No account.<br />Start in 30 seconds.
          </h2>
